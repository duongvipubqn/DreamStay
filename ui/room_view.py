...
class RoomView(ctk.CTkScrollableFrame):
    def __init__(self, master):
        ...
        self.pagination_container = ctk.CTkFrame(
            self.page_frame, fg_color="transparent"
        )
        ...
        ctk.CTkLabel(self.jump_frame, text="Nhảy đến trang:", font=FONT_BODY).pack(
            side="left", padx=5
        )
        self.jump_entry = ctk.CTkEntry(
            self.jump_frame, width=50, height=28, justify="center"
        )
    ...
    def _get_room_status_info(self, room_id):
        ...
        current_booking = any(
            start <= today < end and status != "Cancelled"
    ...
    def load_data(self, filters=None, page=1):
        ...
        if filters:
            ...
            for key, label in mapping.items():
                if label in self.filter_vars:
                    self.filter_vars[label].set(
                        filters.get(key, self.filter_vars[label].get())
                    )
        ...
        if not rooms_db:
            ...
            for i, (r_id, loc, r_type, status, cap, price) in enumerate(rooms_db):
                ...
                ctk.CTkLabel(
                    header_f, text=f"Mã: {r_id}", font=FONT_BODY_BOLD, text_color="#888"
                ).pack(side="left")
                ...
                ctk.CTkLabel(
                    card, text=r_type, font=FONT_LABEL, text_color=COLOR_GOLD
                ).pack(pady=(5, 0))
                ctk.CTkLabel(
                    card, text=f"📍 {loc} | 👥 {cap}", font=FONT_BODY, text_color="#aaa"
                ).pack()
                ctk.CTkLabel(
                    card,
                    text=f"{int(price):,}".replace(",", ".") + " VNĐ / đêm",
                    font=FONT_BODY,
                    text_color=COLOR_GOLD,
                ).pack(pady=10)
                ...
                if upcoming_bookings:
                    ctk.CTkLabel(
                        card,
                        text=f"Lịch đặt: {self._format_booking_summary(upcoming_bookings)}",
                        font=FONT_BODY,
                        text_color="#888",
                    ).pack(padx=20, pady=(0, 10), anchor="w")
                else:
                    ctk.CTkLabel(
                        card, text="Chưa có lịch đặt", font=FONT_BODY, text_color="#888"
                    ).pack(padx=20, pady=(0, 10), anchor="w")
    ...
    def render_pagination(self):
        ...
        pages_to_show.update(
            [self.total_pages - 2, self.total_pages - 1, self.total_pages]
        )

        if self.current_page > 1:
            pages_to_show.add(self.current_page - 1)
        ...
        for p in visible_pages:
            if last_p != 0 and p - last_p > 1:
                ctk.CTkLabel(
                    self.pagination_container, text="...", font=FONT_LABEL
                ).pack(side="left", padx=5)
        ...
        if val.isdigit():
            ...
            if 1 <= page <= self.total_pages:
                ...
                messagebox.showwarning(
                    "Lỗi", f"Vui lòng nhập trang từ 1 đến {self.total_pages}"
                )
        else:
    ...
    def open_booking_modal(self, room_data):
        ...
        ctk.CTkLabel(
            modal, text="ĐẶT PHÒNG NHANH", font=FONT_LABEL, text_color=COLOR_GOLD
        ).pack(pady=20)

        info_f = ctk.CTkFrame(modal, fg_color=COLOR_NAVY, corner_radius=10)
        info_f.pack(fill="x", padx=30, pady=10)
        ctk.CTkLabel(info_f, text=f"Phòng: {room_data[2]}", font=FONT_LABEL).pack(
            pady=5
        )
        ctk.CTkLabel(
            info_f, text=f"Giá: {room_data[5]:,.0f} VNĐ/đêm", font=FONT_BODY
        ).pack(pady=2)
